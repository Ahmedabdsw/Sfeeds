...
public class
UpdateNews extends BroadcastReceiver{
    ...
    /*    public void writeDataToFile(String data) {
            try {
                FileOutputStream outputStreamWriter  = c.openFileOutput("data.txt", Context.MODE_PRIVATE);
        ...
        public void writeDataToFile1(String data) {
            try {
                FileOutputStream outputStreamWriter  = c.openFileOutput("data1.txt", Context.MODE_PRIVATE);
                outputStreamWriter.write(data.getBytes("UTF-8"));
                outputStreamWriter.close();
            }
            catch (IOException e) {
        ...
        public static void writeDataToFile2(String data) {
            try {
                BufferedReader reader1 = null;
                    reader1 = new BufferedReader(new InputStreamReader(c.openFileInput("data2.txt")));
                    String line1 = null;
                    try {
                    ...
                    data = line1+data+";;";
                    FileOutputStream outputStreamWriter  = c.openFileOutput("data2.txt", Context.MODE_PRIVATE);
                outputStreamWriter.write(data.getBytes("UTF-8"));
                outputStreamWriter.close();
            }
            catch (IOException e) {
        ...
        {
            BufferedReader reader1 = new BufferedReader(new InputStreamReader(c.openFileInput("data.txt")));
            BufferedReader reader2 = new BufferedReader(new InputStreamReader(c.openFileInput("data1.txt")));
            String line1 = null;
            String line2 = null;
            //    StringBuffer fileContent = new StringBuffer(""),fileContent1 = new StringBuffer("");
            //          char[] buffer = new char[128];
            line1 = reader1.readLine();
            line2 = reader2.readLine();
    //               Toast.makeText(MainActivity.this, "line1=" + line1.toString() + "  line2=" + line2.toString(), Toast.LENGTH_LONG).show();
            int i=0;M
            String[] line11 = null;
            String[] line22 = null;
            line11 = line1.split("<site>");
            line22 = line2.split("<site>");
            while (i<line11.length) {
                try {
                    if (line11[i].toString().indexOf( line22[i].toString()) == -1)
                        l[i] = i;
                    else
                        l[i] = 0;
                } catch(Exception e){}
    ...
    //        notfication1(l);
      ...
      public static boolean notificationExiste(String s){
            BufferedReader reader1 = null;
            try {
                reader1 = new BufferedReader(new InputStreamReader(c.openFileInput("data2.txt")));
                String line1 = null;
    ...
    public void notfication1(String str[][]){
            if(str.length>0 ) {
                MyNotification[] not = new MyNotification[str.length];
            //    int i = (int) (Math.random() * (str.length-1 - 0)) + 0; // SELECT RANDOM NUMBER
               int i=0; //while(i<str.length-1)
                    not[i] = new MyNotification(c, i,str[i][0], str[i][1]);
            }
    }
 */
    @Override
